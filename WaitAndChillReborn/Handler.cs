        ...
        private IEnumerator<float> LobbyTimer()
        {
            while (!Round.IsStarted)
            {
                ...
                switch (NetworkTimer)
                {
                    ...
                    case 1: message.Replace("%seconds", $"{NetworkTimer + 1} {WaitAndChillReborn.Instance.Config.OneSecondRemain}"); break;

                    default: message.Replace("%seconds", $"{NetworkTimer + 1} {WaitAndChillReborn.Instance.Config.XSecondsRemains}"); break;
                }
